...
class AuthorizationControllerTest extends TestCase
{
    ...
    {
        troojaan\Passport\Passport::tokensCan([
            'scope-1' => 'description',
        ]);
        ...
        $controller = new troojaan\Passport\Http\Controllers\AuthorizationController($server, $response);
        ...
        $authRequest->shouldReceive('getScopes')->andReturn([new troojaan\Passport\Bridge\Scope('scope-1')]);
        ...
        $clients = Mockery::mock('troojaan\Passport\ClientRepository');
        $clients->shouldReceive('find')->with(1)->andReturn('client');

        $tokens = Mockery::mock('troojaan\Passport\TokenRepository');
    ...
    public function test_request_is_approved_if_valid_token_exists()
    {
        ...
        $authRequest->shouldReceive('getScopes')->once()->andReturn([new troojaan\Passport\Bridge\Scope('scope-1')]);
        ...
        $tokens = Mockery::mock('troojaan\Passport\TokenRepository');
        $tokens->shouldReceive('findValidToken')->with($user, 'client')->andReturn($token = Mockery::mock('troojaan\Passport\Token'));
