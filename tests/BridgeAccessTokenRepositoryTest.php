...
class BridgeAccessTokenRepositoryTest extends TestCase
{
    ...
    public function test_access_tokens_can_be_persisted()
    {
        ...
        $tokenRepository = Mockery::mock('troojaan\Passport\TokenRepository');
        ...
        $accessToken = new troojaan\Passport\Bridge\AccessToken(2, [new troojaan\Passport\Bridge\Scope('scopes')]);
        $accessToken->setIdentifier(1);
        $accessToken->setExpiryDateTime($expiration);
        $accessToken->setClient(new troojaan\Passport\Bridge\Client('client-id', 'name', 'redirect'));

        $repository = new troojaan\Passport\Bridge\AccessTokenRepository($tokenRepository, $events);
